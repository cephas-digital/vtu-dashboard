      ...
      <div className="w-full relative lg:left-[200px] md:left-20 left-20">
        <Admin />
         <div className=" lg:flex md:flex grid gap-5 items-center ml-10 mt-10">
                    ...
                    <Link  to="/Standalones" className="border-blue-300 rounded-md border-2 lg:w-[150px]  md:w-[150px] w-[120px] h-[40px]  py-2 text-center font-semibold">
                    Stand-alones
                    </Link>
        ...
        {/* User List Table */}
        <div className='overflow-x-auto md:overflow-x-auto lg:overflow-x-hidden px-4 mx-6'>
          <div 
          ...
          {users.map((user, index) => (
            ...
            grid grid-cols-6 items-center md:gap-10 lg:gap-10 gap-2 lg:w-[1050px] md:w-[1200px] w-[900px] rounded-s-2xl bg-[#fdfdfd] my-1 px-5 py-3">
              <div className="flex items-center gap-2">
                <img src={userIcon} alt="User" className="w-[20px] h-[20px]" />
                <h3>{user.name}</h3>
              </div>
              <h3 className='pl-5' >{user.number}</h3>
              <h3 >{user.phone}</h3>
              <h3>{user.email}</h3>
              <h3>{user.dateJoined}</h3>
              <Link to="/Alluserdetails" className='text-[#3C60FF]'>View User</Link>
            </div>
          ))}
        </div>
